...
{ // compile the definitions and generate gSOAP header file
  collect(definitions);
  fprintf(stream, "/** @mainpage %s Web Service\n\n", definitions.name?definitions.name:"");
  ...
  { if (*op2 && (*op2)->input)
	...
	if (!flag)
      ...
      if ((*op2)->input->multipartRelated)
        fprintf(stream, "  - %s request message has MIME multipart/Related attachments\n", (*op2)->input_name);
      if ((*op2)->input->layout)
        fprintf(stream, "  - %s request message has DIME attachments in compliance with\n  %s\n\n", (*op2)->input_name, (*op2)->input->layout);
      if ((*op2)->output && (*op2)->output_name && (*op2)->output->multipartRelated)
        fprintf(stream, "  - %s response message has MIME multipart/Related attachments\n", (*op2)->output_name);
      if ((*op2)->output && (*op2)->output_name && (*op2)->output->layout)
        fprintf(stream, "  - %s response message has DIME attachments in compliance with\n  %s\n\n", (*op2)->output_name, (*op2)->output->layout);
      fprintf(stream, "*/\n\n");
      (*op2)->generate(types);
    }
  }
