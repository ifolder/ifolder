...
*/
		...
		/// <summary>
		/// Returns an array of the Unsynchronized Nodes for a specified collection.
		/// </summary>
		/// <returns></returns>
		[WebMethod(EnableSession=true, Description="Returns an array of the Unsynchronized Nodes for a specified collection.")]
		[SoapDocumentMethod]
		public UnsynchronizedNode[] GetUnsynchronizedNodes(string collectionID)
		{
			ArrayList list = new ArrayList();
			...
			// Add on the nodes with a "SyncStatus" property
			ICSList unsynchronizedNodeList = col.GetUnsynchronizedNodes();
			foreach(ShallowNode sn in unsynchronizedNodeList)
			{
				Node node = col.GetNodeByID(sn.ID);
				UnsynchronizedNode unsynchronizedNode =
					new UnsynchronizedNode(node);
					
				list.Add(unsynchronizedNode);
			}

			return (UnsynchronizedNode[])list.ToArray(typeof(UnsynchronizedNode));
		}
		
		/// <summary>
		/// Returns an array of the Ignored Unsynchronized Nodes for a specified collection.
		/// </summary>
		/// <returns></returns>
		[WebMethod(EnableSession=true, Description="Returns an array of the Ignored Unsynchronized Nodes for a specified collection.")]
		[SoapDocumentMethod]
		public UnsynchronizedNode[] GetIgnoredUnsynchronizedNodes(string collectionID)
		{
			ArrayList list = new ArrayList();
			...
			ICSList ignoredNodes = col.GetIgnoredUnsynchronizedNodes();
			foreach(ShallowNode sn in ignoredNodes)
			{
				Node node = col.GetNodeByID(sn.ID);
				UnsynchronizedNode unsynchronizedNode =
					new UnsynchronizedNode(node);
		...
		/// <summary>
		/// Mark the specified node to be ignored (not returned by the
		/// GetUnsynchronizedNodes() method and returned by the
		/// GetIgnoredNodes() method).
		/// </summary>
		/// <returns></returns>
		[WebMethod(EnableSession=true, Description="Mark the specified node to be ignored (not returned by the GetUnsynchronizedNodes() method and returned by the GetIgnoredNodes() method).")]
		[SoapDocumentMethod]
		public void IgnoreUnsynchronizedNode(string collectionID, string nodeID, bool ignore)
		{
			Collection col = Store.GetStore().GetCollectionByID(collectionID);
			if (col == null)
				throw new Exception("Invalid Collection ID");
			...
			Property p = node.Properties.GetSingleProperty("IgnoreSyncStatus");
			if (ignore && p == null)
			{
				// Make sure that the specified node had a Sync problem or is
				// a collision.  If not, throw an error saying that this is an
				// invalid node to ignore.
				if (node.Properties.GetSingleProperty(PropertyTags.SyncStatusTag) == null
					&& node.Properties.GetSingleProperty(PropertyTags.Collision) == null)
					throw new Exception("This node is not an unsynchronized node");

				// Add the "IgnoreSyncStatus" property
				p = new Property("IgnoreSyncStatus", true);
			...
			if (!ignore && p != null)
			{
				// Delete the "IgnoreSyncStatus" property
		...
		/// <summary>
		/// Returns the total number of unsynchronized nodes for the specified
		/// collection.
		/// </summary>
		/// <returns></returns>
		[WebMethod(EnableSession=true, Description="Returns the total number of unsynchronized nodes for the specified collection.")]
		[SoapDocumentMethod]
		public int GetUnsynchronizedNodeCount(string collectionID)
		{
			ArrayList list = new ArrayList();
			...
			ICSList unsynchronizedNodeList = col.GetUnsynchronizedNodes();
			return unsynchronizedNodeList.Count;
		}
	}
	...
	/// </summary>
	[Serializable]
	public class UnsynchronizedNode
	{
		/// <summary>
		/// The ID of the Simias Node
		/// </summary>
		public string ID;
		
		/// <summary>
		/// The type of the Simias Node.  One of: DirNode, FileNode, Policy, Node
		/// </summary>
		public string Type;

		/// <summary>
		/// The Name of the Simias Node
		/// </summary>
		public string Name;

		/// <summary>
		/// The SyncStatus of the Simias Node
		/// </summary>
		...
		/// <summary>
		/// Constructs an UnsynchronizedNode object.
		/// </summary>
		/// <param name="node">The ID of the node that represents the Unsynchronized Node.</param>
		public UnsynchronizedNode(Node node)
		{
			this.ID = node.ID;
			this.Type = node.Type;
			this.Name = node.Name;
			
			Property p = node.Properties.GetSingleProperty( PropertyTags.SyncStatusTag );
			if ( p != null )
				this.SyncStatus = (string) p.Value;
			else if ( node.Properties.GetSingleProperty( PropertyTags.Collision ) != null )
				this.SyncStatus = "Collision";
			else
