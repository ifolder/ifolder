/***********************************************************************
 *  PropertySheet.cpp - Implements the IShellPropSheetExt member
 *  functions.  The IShellPropSheetExt interface allows a property sheet
 *  handler to add or replace pages in the property sheet displayed for 
 *  a file object.
 * 
		...
		case BN_CLICKED:
			if (resourceId == IDC_ADVANCED)
			{
				...
				try
				{
					...
					if (lpciFolderShell->m_spiFolder)
					{
						lpciFolderShell->m_spiFolder->InvokeAdvancedDlg(lpciFolderShell->m_szFileUserClickedOn);
					}
				}
				catch (...)
				{
