...
ifndef PRODUCT_NAME
  ERROR_MESSAGE += "ERROR: PRODUCT_NAME must be set (ex: addressbook)\n"
endif
...
# Targets
#=============================================================================
# Always rebuild the install package; we don't know what files may have changed
all: ShutdowniFolder\ShutdowniFolder.exe
	$(header)
	$(RUN_DEVENV) $(SLN_FILE) $(PROJECT) $(CONFIGURATION)
	copy /y $(PROJECT)\\$(CONFIGURATION)\\$(SIMPLE_MSI_FILENAME) $(FULL_MSI_FILENAME)

ShutdowniFolder\ShutdowniFolder.exe: $(SLN_FILE) ShutdowniFolder\ShutdowniFolder.cs
	$(header)
	$(RUN_DEVENV) $(SLN_FILE) ShutdowniFolder $(CONFIGURATION)

# devenv creates both Release and Debug directories when building either one so remove both
clean:
	$(header)
	-$(RMDIR) client\Release
	-$(RMDIR) client\Debug
	-$(RMDIR) server\Release
	-$(RMDIR) server\Debug
	-$(RMDIR) ShutdowniFolder\bin
	-$(RMDIR) ShutdowniFolder\obj
	-$(RM) *-$(PROJECT).log
	-$(RM) *-$(PROJECT)-*.msi
	-$(RM) /ah installs.suo
	-$(RM) ShutdowniFolder\*.exe
	-$(RM) *-ShutdowniFolder.log
...
#
# $Log$
# Revision 1.1  2004/02/24 18:25:28  pthomas707
# Morphed Denali MSI build into iFolder MSI build.
#
#
#=============================================================================
