...
//		private Gtk.MenuItem		ConflictMenuItem;
		private Gtk.MenuItem		UnsynchronizedMenuItem;
...
//		private Button				ResolveConflictsButton;
		private Button				UnsynchronizedNodesButton;
...
//			ConflictMenuItem.Sensitive = false;
			UnsynchronizedMenuItem.Sensitive = false;
...
//					new EventHandler(OnResolveConflicts);
			...
			UnsynchronizedMenuItem = new MenuItem (Util.GS("View unsynchronized files"));
			iFolderMenu.Append(UnsynchronizedMenuItem);
			UnsynchronizedMenuItem.Activated +=
					new EventHandler(OnViewUnsynchronizedNodes);
...
*/
			...
			///
			/// Unsynchronized Nodes Button
			///
			hbox = new HBox(false, 0);
			UnsynchronizedNodesButton = new Button(hbox);
			vbox.PackStart(UnsynchronizedNodesButton, false, false, 0);
			UnsynchronizedNodesButton.Relief = ReliefStyle.None;

			buttonText = new Label(
				string.Format("<span size=\"large\">{0}</span>",
							  Util.GS("View unsynchronized files...")));
			hbox.PackStart(buttonText, false, false, 4);
			buttonText.UseMarkup = true;
			buttonText.UseUnderline = false;
			buttonText.Xalign = 0;
			
			UnsynchronizedNodesButton.Clicked +=
				new EventHandler(OnViewUnsynchronizedNodes);
...
//		}
		...
		private void OnViewUnsynchronizedNodes(object o, EventArgs args)
		{
			ViewSelectedFolderUnsynchronizedNodes();
		}
...
*/
						if(holder.iFolder.HasUnsynchronizedNodes)
						{
							MenuItem item_unsynchronized_nodes = new MenuItem (
									Util.GS("View unsynchronized files..."));
							menu.Append (item_unsynchronized_nodes);
							item_unsynchronized_nodes.Activated += new EventHandler(
								OnViewUnsynchronizedNodes);
...
//					ResolveConflictsButton.Visible			= false;
					UnsynchronizedNodesButton.Visible		= false;
...
*/
					if (holder.iFolder.HasUnsynchronizedNodes)
						UnsynchronizedNodesButton.Visible = true;
					else
						UnsynchronizedNodesButton.Visible = false;
				}
...
//					ConflictButton.Sensitive = true;
					UnsynchronizedMenuItem.Sensitive = true;
					UnsynchronizedNodesButton.Sensitive = true;
				}
				else
				{
//					ConflictMenuItem.Sensitive = false;
//					ConflictButton.Sensitive = false;
					UnsynchronizedMenuItem.Sensitive = false;
					UnsynchronizedNodesButton.Sensitive = false;
				}
...
//				ConflictMenuItem.Sensitive = false;
		...
		private void ViewSelectedFolderUnsynchronizedNodes()
		{
			iFolderHolder holder = iFoldersIconView.SelectedFolder;
			if (holder != null)
				ShowFolderProperties(holder, 2);
		}
/*
