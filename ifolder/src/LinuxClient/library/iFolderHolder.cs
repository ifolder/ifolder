...
namespace Novell.iFolder
{
	...
	public class iFolderHolder
	{
		...
		private int			unsynchronizedNodeCount;
...
//		private static bool unitTested = false;
		...
		public iFolderHolder(iFolderWeb ifolder)
		{
			...
			this.unsynchronizedNodeCount = 0;
			UpdateDisplayData();
...
*/
		...
		public int UnsynchronizedNodeCount
		{
			get
			{
				return unsynchronizedNodeCount;
			}
			set
			{
				unsynchronizedNodeCount = value;
				UpdateDisplayData();
			}
		}
...
//				{
					switch (state)
					{
						...
						case iFolderState.FailedSync:
							if (ifolder.HasUnsynchronizedNodes)
								stateString = string.Format(Util.GS("{0} items did not synchronize"), unsynchronizedNodeCount);
							else
