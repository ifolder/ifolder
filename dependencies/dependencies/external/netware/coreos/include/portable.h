...
**	PORTABLE.H
**
**			   Portable.h provides definitions that support C code
**			   portablity in various environments.
*/
 ...
 *		machine type.
 *
 *		Note: you should define one of these machine types in your
 *		build scripts before including portable.h.
 */

#ifdef	IAPX386	/* INTEL x86 */
#define 	   LO_HI_MACH_TYPE
#endif

#ifdef	MC680X0	/* MOTOROLA 68K */
...
/*
**	The following provides definitions for primitive integer
**	data types that may change depending on the target
**	platform (P5, P6, Merced).  These types were adopted
**	by committee on 18Dec96 and are intended to supercede
**	the BYTE, WORD,	and LONG data types currently defined
**	in portable.h, etc.
*/

/**********************************************/
/*     UINT8 is an unsigned 8-bit integer.    */
/*     It replaces the original BYTE type.    */
/**********************************************/
#define UINT8	unsigned char

/**********************************************/
/*    UINT16 is an unsigned 16-bit integer.   */
/*     It replaces the original WORD type.    */
/**********************************************/
#define UINT16	unsigned short

/**********************************************/
/*    UINT32 is an unsigned 32-bit integer.   */
/*    It replaces some of the uses for the    */
/*  original LONG type.  UINT32 should ONLY   */
/*  be used for storing 4-byte data elements. */
/**********************************************/
#ifdef IAPX386
...
/*   It should only be used where very large  */
/*   unsigned integers are required, such as  */
/*    for file offsets in file systems that   */
/*   support large (>4GB) files.  UINT64 is   */
/*   equivalent to the NAFS QUAD data type.   */
/**********************************************/
#ifdef IAPX386
#ifdef __MWERKS__			// metrowerks compiler 64-bit type
...
/**********************************************/
/* UINT is an unsigned integer data type that */
/*   is guaranteed to be 32-bits or greater.  */
/*   The size is processor specific and will  */
/*      be selected to maximize processor     */
/*    performance.  UINT should be used for   */
/*    items such as counts, array indexes,    */
/*             return codes, etc.             */
/**********************************************/
#define UINT	unsigned int

/**********************************************/
/*      SINT8 is a signed 8-bit integer.      */
/*  Since characters in NetWare are unsigned  */
/*  quantities, the use of SINT8 is expected  */
/*          to be extremely limited.          */
/**********************************************/
#define SINT8	signed char

/**********************************************/
/*     SINT16 is a signed 16-bit integer.     */
/*     The use of SINT16 is expected to be    */
/*             extremely limited.             */
/**********************************************/
#define SINT16	short

/**********************************************/
/*     SINT32 is a signed 32-bit integer.     */
/*    Use SINT unless you absolutely need a   */
...
/**********************************************/
/*     SINT64 is a signed 64-bit integer.     */
/*   It should only be used where very large  */
/*   integers are required, such as for file  */
/*    offsets in file systems that support    */
/* large (>4GB) files.  SINT64 is equivalent  */
/*        to the NAFS SQUAD data type.        */
/**********************************************/
#ifdef IAPX386
#ifdef __MWERKS__ 			// metrowerks 64-bit type
...
/**********************************************/
/*   SINT is a signed integer data type that  */
/*   is guaranteed to be 32-bits or greater.  */
/*   The size is processor specific and will  */
/*      be selected to maximize processor     */
/*    performance.  SINT should be used for   */
/*    items such as counts, array indexes,    */
/* return codes, etc. that could go negative. */
/**********************************************/
#define SINT	int

/**********************************************/
/*    ADDR is an unsigned integer data type   */
/*   whose size is large enough to represent  */
/*   a logical byte address.  Arithmetic and  */
/*   bitwise operations may be performed on   */
/*  ADDR data types, since they are integers. */
/* Use the void* type (or other pointer type) */
/*   for addresses that may be dereferenced.  */
/**********************************************/
#ifdef IAPX386
...
/**********************************************/
/*  STR is equivalent to UINT8 and should be  */
/* used to define pointers to NULL-terminated */
/* ASCII strings.  STR is NOT a pointer type, */
/*     so the "*" must be included in each    */
...
/**********************************************/
/*  LSTR is equivalent to UINT8 and should be */
/* used to define pointers to length-preceded */
/*    ASCII strings.  LSTR is NOT a pointer   */
/*  type so the "*" must be included in each  */
/*          definition.  For example:         */
/*   LSTR *foo, bar[] = {4,'t','e','s','t'};  */
/**********************************************/
#define LSTR	unsigned char

/**********************************************/
/* LSTRZ is equivalent to UINT8 and should be */
/*   used to define pointers to strings that  */
/*  are length-preceded AND NULL-terminated.  */
/* LSTR is NOT a pointer type so the "*" must */
/*       be included in each definition.      */
/*                For example:                */
/* LSTRZ *foo, bar[] = {4,'t','e','s','t',0}; */
/**********************************************/
#define LSTRZ	unsigned char


/*************************** OBSOLETE TYPES *************************/
/*
** The following types should not be used for new code, and existing
** code that uses these types should be changed to use one of the
** types defined in the previous section of this file.
*/
#ifndef LONG
 ...
 ****************************************************************************
 *
 * The next set of macros will be used less frequently but is included for
 ...
 *
 *	Purpose:
 *		Useful for copying sockets, networknumbers, nodenumbers or complete
 *		ipx addresses between ipx packets or between ipx packets and some
 *		location in memory that is in hilo order.
 *
 *	Example:
 *		..
...
typedef struct {
	BYTE bytes[2];
} _C2T_;	/* Copy 2 Typedef */

typedef struct {
	BYTE bytes[4];
} _C4T_;	/* Copy 4 Typedef */

typedef struct {
	BYTE bytes[6];
} _C6T_;	/* Copy 6 Typedef */

typedef struct {
	BYTE bytes[12];
} _C12T_;	/* Copy 12 Typedef */
/*
 ...
 */
/*
 * If this machine's byte order is hilo and it is not
 * strictly aligned then do a simple fetch.
 * Otherwise do it a byte at a time.
 */
#if defined(HI_LO_MACH_TYPE) && !defined(STRICT_ALIGNMENT)
#define GET_HILO_WORD(addr) (*(WORD *)(addr))
#else
#define GET_HILO_WORD(addr) ((WORD) (			\
	((BYTE *)(addr))[0] << 8 |					\
	((BYTE *)(addr))[1] 						\
))
#endif
...
#else
#define GET_HILO_LONG(addr) ((LONG) (									\
	((BYTE *)(addr))[0] << 24 | 										\
	((BYTE *)(addr))[1] << 16 | 										\
	((BYTE *)(addr))[2] << 8 |											\
	((BYTE *)(addr))[3] 												\
...
/*
 * If this machine's byte order is hilo and it is not
 * strictly aligned then do a simple store.
 * Otherwise do it a byte at a time.
 */
#if defined(HI_LO_MACH_TYPE) && !defined(STRICT_ALIGNMENT)
 ...
 */
/*
 * If this machine's byte order is lohi and it is not
 * strictly aligned then do a simple fetch.
 * Otherwise do it a byte at a time.
 */
 ...
 *
 *	Description:
 *		Given a word in host byte order, return its value in hilo order.
 *		Parameter may be a variable or constant.
 *
 *	Purpose:
 *		For converting values not in memory (ie constants, register variables
 *		and expressions) to hilo order.
 *
 *	Examples:
 ...
 *
 *	Description:
 *		Given a long in host byte order, return its value in hilo order.
 *		Parameter may be a variable or constant.
 *
 *	Purpose:
 *		For converting values not in memory (ie constants, register variables
 ...
 *	Example:
 *		..
 *		LONG internalnetnumberinhilo = VALUE_TO_HILO_LONG(netnumberinhostorder);
 *		..
 *
 *	Note:
 ...
 *	Example:
 *		..
 *		LONG internalnetnumberinlohi = VALUE_TO_LOHI_LONG(netnumberinhostorder);
 *		..
 *
 *	Note:
 *		Beware of side affects since long may be evaluated twice.
 ...
 *
 *	Note:
 *		On machines where no swapping is needed this turns into a byte copy.
 *		Notice that the COPY_TO and COPY_FROM are identical.
 */
#if defined(HI_LO_MACH_TYPE)
 ...
 *
 *	Purpose:
 *		Useful for assuring the correct endianess of a value found at addr.
