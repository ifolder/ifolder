...
namespace Simias.Storage
{
	...
	public class PropertyList : IEnumerable, ISerializable
	{
		...
		internal PropertyList( Node node )
		{
			...
			propertyRoot.SetAttribute( Property.NameAttr, node.Name );
			propertyRoot.SetAttribute( Property.IDAttr, node.Id );
			propertyRoot.SetAttribute( Property.TypeAttr, node.Type );
		...
		/// <param name="listState">Current state of property list.</param>
		internal PropertyList( XmlElement xmlProperties, PropertyListState listState )
		{
			nodeDocument = xmlProperties.OwnerDocument;
			propertyRoot = xmlProperties;
			state = listState;
		}
		...
		protected PropertyList ( SerializationInfo info, StreamingContext context )
		{
			...
			propertyRoot = nodeDocument.DocumentElement[ Property.ObjectTag ];
