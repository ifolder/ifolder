	...
	public sealed class Store : IEnumerable, IDisposable
	{
		...
		private Store( Uri databasePath, string componentId )
		{
			...
			lock( this )
			{
				...
				identityManager = IdentityManager.CreateStoreAdmin( this, domainName );
		...
		private void AuthenticateStore()
		{
			...
			identityManager = new IdentityManager( this, identity );
		}
		...
		/// <summary>
		/// Callback that handles the update of the cached node table when a node changes.
		/// </summary>
		...
		private void OnNodeChanged( NodeEventArgs args )
		{
			try
			{
				...
				{
/*
					// If the event was generated by this store handle, ignore it.
					if ( Instance != args.EventId )
					{
						// Get the node that changed.
						lock ( this )
						{
							cacheNodeTable.Remove( args.ID );
						}
					}
*/					
				}
