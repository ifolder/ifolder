	...
	public class NotLogging 
	{
		...
		private static int ProgramInit(String[] args) 
		{
			...
			else if ("true" == args[0]) 
			{
				...
				((Hierarchy)LogManager.GetLoggerRepository()).Threshold = log4net.spi.Level.WARN;
			} 
			else 
			{
				Usage();
			...
			((Logger)SHORT_LOG.Logger).Level = log4net.spi.Level.INFO;
			((Hierarchy)LogManager.GetLoggerRepository()).Root.Level = log4net.spi.Level.INFO;
