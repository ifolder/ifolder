...
namespace ABGui
{
	...
	{
	//	static public	Contact					cContact = null;
		...
		public ABGui()
		{
			...
			//
			// Get an instance to the Address Book manager
			// Open the local contact list and enumerate all the contacts - adding them to the list box
			//
			...
			// Test if this is the default address book
			bool isDefault = addressBook.Default;

			//this.contacts.DisplayMember = "UserName";
		...
		{
			// Bring the contact back from the store to catch any 
			// out of band changes
			...
			Contact cContact = addressBook.GetContactByIdentity(identity);
			*/
			...
			if (results == false)
			{
				// This guy didn't have a contact so don't show the box
		...
		private void saveContact_Click(object sender, System.EventArgs e)
		{
			...
			if (username != null)
			{
				// If the id field is null then the save was for a new contact
				// if hte id field is non-null then the same is an update of
				// an existing contact
				...
				if (editID.Text == null || editID.Text == "")
				{
					...
					{
						/*
						Email tmpMail = new Email((EmailTypes.internet | EmailTypes.work), bMailEdit.Text);
						tmpMail.Preferred = true;
						contact.AddEmailAddress(tmpMail);
						*/

						// Test the shortcut method
		...
		private void editSearch_TextChanged(object sender, System.EventArgs e)
		{
			...
			if (editSearch.Text == "*")
			{
				...
				{
					//this.contacts.Items.Add(contact2);
					switch(listBy)
					{
						case "Username":
						{
						...
						case "E-Mail":
						{
							if (contact2.EMail != "")
							{
								...
								this.contacts.Items.Add(cLite);
								//this.contacts.Items.Add(contact2.EMail);
							}
							break;
						}
			...
			else
			{
				...
				if (listBy == "Username")
				{
					...
					//contactList = addressBook.SearchUsername(editSearch.Text, queryType);
				}
				else
				if (listBy == "E-Mail")
				{
				...
				try
				{
					...
					// Load the preferred address fields into the GUI
					LoadAddresses(contact);

					if( LoadContactPicture(contact) == false)
					{
						// This guy didn't have a photo so don't show the box
						contactPicture.Visible = false;
					}
				}
				catch{}
		...
		private void exportButton_Click(object sender, System.EventArgs e)
		{
			...
			if (contact != null)
			{
				...
				//openFileDlg.FileOk += new System.ComponentModel.CancelEventHandler(openFileDlg_OK);
				if(saveDlg.ShowDialog() == DialogResult.OK)
				{
					//string fileName = "vcard_for_" + contact.FN + ".vcf";
					//saveDlg.FileName
		...
		private bool SaveHomeAddress(Contact contact)
		{
			...
			// An address requires a valid zip code
			if (hZipEdit.Text != null && hZipEdit.Text != "")
			{
				...
				try
				{
					// First see if there is a home address
					...
					if (homeExists == true)
					{
						// OK - we already have a HOME address so just update
		...
		private bool SaveWorkAddress(Contact contact)
		{
			...
			if (bZipEdit.Text != null && bZipEdit.Text != "")
			{
				...
				try
				{
					...
					if (workExists == true)
					{
						// OK - we already have a WORK address so just update
		...
		public static void openFileDlg_OK(object sender, System.ComponentModel.CancelEventArgs e)
		{
			...
			ABGui gui = (ABGui) sender;

			if (gui.editID.Text != null && gui.editID.Text != "")
			{
				Contact contact = gui.abContactList.GetContact(gui.editID.Text);
				Photo photo = contact.GetPhoto();
				Stream pStream = photo.Create();
			}
			*/
		}
		...
		private void comboBox1_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			if (this.comboListBy.Text != listBy)
			{
				...
				contacts.Items.Clear();
				//this.comboListBy.Items.Clear();

				if (listBy == "Username")
				{
					//contacts.DisplayMember = "UserName";
					foreach(Contact contact in addressBook)
					{
//						this.contacts.Items.Add(contact);
						this.contacts.Items.Add(new ContactLite(contact.UserName, contact.ID));
					}
				}
				else
		...
		private void vCardImport_Click(object sender, System.EventArgs e)
		{
			...
			if(openFileDlg.ShowDialog() == DialogResult.OK)
			{
				...
				if (newContact != null)
				{
					// vCard doesn't have support for typing an e-mail address to
					// work, home etc.  AddressBook defaults preferred e-mail addresses
					// to type "work" and the rest to type "other".  This app only supports
					// two e-mail addresses: one is work and the other personal.  If we have
					// more than one e-mail address we need to set one of the "other" to
					// "personal".
	...
	// Small object that holds a viewable property and the contact ID
